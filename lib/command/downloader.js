  ...
  async (msg, { query, client }) => {
    if (!query)
      return client.sendText(
        msg.from,
        `Isi Query!\nContoh : #tiktok https://www.tiktok.com/@bulansutena/video/6922257908973554945`,
        msg
      );
try {
let res = await functions.axios.get("https://api.xteam.xyz/dl/tiktok?url="+query+"&APIKEY=1c3d6537ad6e7050")
if (res.data.code !== 200) return client.reply(msg, "Link yang kamu kirim tidak valid")
client.sendMessage(
      msg.from,
      (await client.getBuffer(res.data.result.link_dl1)).buffer,
      "videoMessage", { caption:res.data.result.caption+"\n*@"+res.data.result.name+"*" }
      )
} catch {
client.reply(msg, "Unexpected Error!!")
}
  },
  { wait: true, param: functions.needed("Link Tiktok") }
);
