...
async (msg, res) => {
	const txti = res
		for (let i = 0; i < txti.length; i++) {
			 client.sendText(msg.from, txti.charAt(i));
		},
	},
);
...
cmd.on(
  ...
  ["owner"],
  async (msg, res) => {
    let parse = res.command.includes("=>")
      ? res.text.replace("=>", "return ")
      : res.text.replace(">", "");
    try {
      ...
      return client.reply(msg, functions.util.format(evaluate), {
        ...
        contextInfo: {
          externalAdReply: {
            ...
            sourceUrl: "https://github.com/Zacros-Team/ZacrosBot/",
          },
        },
      });
    } catch (e) {
  ...
  async (msg, res) => {
    try {
      functions.exec(`${res.query}`, (err, out) => {
        if (err)
          return client.reply(msg, functions.util.format(err), {
            quoted: msg,
            contextInfo: {
              externalAdReply: {
                title: botinfo.botname,
...
cmd.on(
  ...
  ["owner"],
  (msg, { query, client }) => {
    let anu = client.chats
      .all()
      .filter((a) => !a.read_only && a.message && !a.archive)
      .map((a) => a.jid);
    for (let _ of anu) {
...
cmd.on(
  ...
  ["owner"],
  (msg, { query, client }) => {
    let asu = client.chats
      .all()
      .filter(
        (v) =>
          v.jid.endsWith("g.us") && !v.read_only && v.message && !v.announce
      )
      .map((v) => v.jid);
    for (let f of asu) {
      ...
      case "name":
        {
          ...
          return client.reply(
            msg,
            `Sukses Mengganti Nama WhatsApp Bot : ${qq[1]}`
          );
        }
        break;
      case "bio":
        {
          ...
          return client.reply(
            msg,
            `Sukses Mengganti Bio WhatsApp Bot : ${qq[1]}`
          );
        }
        break;
      case "prefix":
      ...
      case "ownername":
        {
          ...
          return client.reply(
            msg,
            `Sukses Mengganti Nama Owner Bot : ${qq[1]}`
          );
        }
        break;
      case "footertext":
        {
          ...
          return client.reply(
            msg,
            `Sukses Mengganti Default Footer Button : ${qq[1]}`
          );
        }
        break;
    }
