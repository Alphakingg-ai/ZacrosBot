cmd.on(
  ...
  async (msg, { client }) => {
  try {
    mt = await client.groupMetadata(msg.from)
    un = "⬣ ━━━━━━━━━ ⬣"
    teks = un+"\n"
     for (let i of mt.participants) {
     teks += `⬡  @${i.jid.split("@")[0]}\n`
     }
   client.sendMessage(msg.from, teks+un, "conversation", {contextInfo: {mentionedJid: functions.parseMention(teks)}})
  } catch {
   client.reply(msg, "Unexpected Error!!!");
   }
  },
  {
    admin: true,
...
cmd.on(
  ...
  ["add"],
  ["group"],
  async (msg, { client, query }) => {
    let intNumber = query.replace(/[^0-9]/g, "") + "@s.whatsapp.net";
    if (query.replace(/[^0-9]/g, "").startsWith("0"))
      return client.reply(
        msg,
        "*REJECTED* | Masukan Nomor Valid Dengan Kode Negara Contoh +62 / 62"
      );
    let checkNumber = await client.isOnWhatsApp(intNumber);
    if (!checkNumber)
      return client.reply(
        msg,
        "*REJECTED* | Nomor Yang Anda Masukan Tidak Terdaftar Di Whatsapp"
      );
    ...
    if (statusInt == 409)
      return client.reply(msg, "*FAILED* | Member Telah Ada Disini");
    else if (statusInt == 403) {
      client.reply(
        msg,
        `*FAILED* | @${
          intNumber.split("@")[0]
        } Tidak Bisa Di Invite Karena Di Private!, Mengirim Link Grub....`,
        {
          contextInfo: {
      ...
      return client.sendMessage(
        ...
        "https://chat.whatsapp.com/" +
          inviteId +
          `\n Kamu Di Invite Oleh @${msg.sender.jid.split("@")[0]}`,
        "conversation",
        {
          contextInfo: {
...
cmd.on(
  ...
  ["kick"],
  ["group"],
  async (msg, { client, query, prefix }) => {
    if (!msg.mentionedJid && !query)
      return client.reply(
        msg,
        `*REJECTED* | Tag Seseorang Atau Masukan No Yang Akan Di Kick`
      );
    if (!msg.mentionedJid && query) {
      if (query.replace(/[^0-9]/g, "").startsWith("0"))
        return client.reply(
    ...
    for (let a of msg.mentionedJid) {
      ...
      if (find && find.isAdmin) {
        ...
        await client.sendMessageFromContent(
          msg.from,
          {
            buttonsMessage: {
              footerText: `Jika Tidak Bisa Tekan Tombol, Ketik ${prefix[0]}kick @Tag Orang --force`,
              contentText:
                "*WARNING* | Yang Di Kick Adalah Admin Apakah Anda Yakin?",
              buttons: [
                {
                  buttonId: `${prefix[0]}kick ${a.split("@")[0]} --force`,
                  buttonText: {
                    displayText: "Ya",
                  },
                  type: 1,
...
cmd.on(
  ...
  async (msg, { client }) => {
    try {
      let demoteGroup = await client.groupDemoteAdmin(
        msg.from,
        msg.mentionedJid
      );
...
cmd.on(
  ...
  async (msg, res) => {
    try {
      let promoteGroup = await client.groupMakeAdmin(
        msg.from,
        msg.mentionedJid
      );
...
cmd.on(
  ...
  async (msg, res) => {
    try {
      ...
      if (groupSubject.status !== 200)
        return client.reply(
          msg,
          "some error when trying changing group subject"
        );
      client.reply(
        msg,
        "Success change group Name" +
...
cmd.on(
  ...
  async (msg, res) => {
    try {
      ...
      client.reply(
        msg,
        "Success change group Description\n\nFrom: " +
          res.group.desc +
          "\nTo: " +
          nameInt
      );
  ...
  async (msg, res) => {
    client
      ...
      .then((response) => {
        client.sendMessage(
          msg.sender.jid,
          "Success Keluar dari group " + res.group.subject,
          "conversation"
        );
      })
...
cmd.on(
  ...
  ["group", "owner"],
  async (msg, res) => {
    let arrInt = [msg.sender.jid, ...msg.mentionedJid];
    let groupName = res.query.split(/ @/)[0];
    client
      .groupCreate(groupName, arrInt)
      .then((response) => {
        client.reply(
          msg,
          "groupId: " +
            ...
            "\n\nSuccess membuat group dengan nama " +
            groupName
        );
      })
      .catch((err) => client.reply(msg, functions.util.format(err)));
...
cmd.on(
  ...
  async (msg, res) => {
    try {
      ...
      client.reply(
        msg,
        "Link Group baru: https://chat.whatsapp.com/" + revokeResponse.code
      );
    } catch (err) {
      client.reply(msg, functions.util.format(err));
    }
...
cmd.on(
  ...
  ["group"],
  async (msg, res) => {
    let response = await client.groupSettingChange(
      msg.from,
      "announcement",
      true
    );
...
cmd.on(
  ...
  ["group"],
  async (msg, res) => {
    let isValidUrl = res.query.match(
      new RegExp(/(http[s]?:\/\/.*\.(?:png|jpg|gif|svg|jpeg))/, "i")
    );
    if (isValidUrl) {
      ...
      client.sendImage(
        msg.from,
        res.query,
        "Success update Group Profile Picture!",
        msg
      );
    } else {
...
cmd.on(
  ...
  async (msg, { query, client }) => {
    return client.reply(msg, query, {
      contextInfo: {
        mentionedJid: msg.groupData.participants.map((tr) => tr.jid),
      },
    });
  },
  {
...
cmd.on(
  ...
  ["group", "useless"],
  async (msg, { client, query, prefix }) => {
    if (!groupDb[msg.from]) groupDb[msg.from] = {};
    if (
      !query ||
      !["nyala", "nyalain", "on", "mati", "matiin", "off"].includes(query)
    ) {
      let buttons = [
        {
      ...
      return client.sendMessageFromContent(msg.from, {
        buttonsMessage: {
          footerText: `Jika Tidak Bisa Menekan Tombol, Ketik ${prefix[0]}welcome nyala atau ${prefix[0]}welcome mati`,
          contentText: "Pilih Kak Nyalain Atau Matiin Welcome",
          buttons,
          headerType: "EMPTY",
        },
      });
    ...
    if (["nyala", "nyalain", "on"].includes(mode)) {
      if (groupDb[msg.from]?.welcome)
        return client.reply(msg, `*REJECTED* | Sudah Nyala Dari Tadi`);
      ...
      return client.reply(
        msg,
        `*SUCCESS* | Sukses Memasukan Ke Database Welkom!`
      );
    } else if (["mati", "matiin", "off"].includes(mode)) {
      if (!groupDb[msg.from]?.welcome)
        return client.reply(msg, `*REJECTED* | Sudah Mati Dari Tadi`);
...
cmd.on(
  "welkom",
  ["setwelcome", "setwelkom", "setwelcom"],
  ["group", "useless"],
  (msg, { query, prefix, client }) => {
    if (!groupDb[msg.from]?.welcome)
      return client.reply(msg, `*REJECTED* | Welkom Nya Mati`);
    if (!query.includes("|"))
      return client.reply(
        msg,
        `*REJECTED* | Format Salah Harusnya ${prefix[0]}setwelcome type|isinya contoh ${prefix[0]}setwelcome promote|cie di promote %user_tag`
      );
    let type = query.split("|")[0].toLowerCase().trim();
    let isinya = query.split("|")[1];
    if (!Object.keys(groupDb[msg.from].welcome).includes(type))
      return client.reply(
        msg,
        `*REJECTED* | Type Yang Anda Masukan Tidak Valid, Berikut Type Type Nya ${Object.keys(
          groupDb[msg.from].welcome
        )}`
      );
    ...
    return client.reply(
      msg,
      `*SUCCESS* | Sukses Mengubah Data Welcome ${type} Ke ${isinya}`
    );
  },
  {
    group: true,
