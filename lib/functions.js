...
exports.Functions = class Functions {
  ...
  strong() {
    let string =
      "1234567890ABCDEFGIJKLMNOPQRSTUVWXYZabcdefgijklmnopqrstuvwxyz`~!@#$%^&*()_+=";
    ...
    let chatMeta = async (mess) => {
      ...
      mess.data =
        typeof mess.message[mess.type] == "object"
          ? Object.keys(mess.message[mess.type]).includes("contextInfo")
      ...
      mess.string =
        mess.type === baileys.MessageType.text
          ? mess.message.conversation
          : mess.data.includes("caption")
          ? mess.message[mess.type].caption
          : mess.type == baileys.MessageType.extendedText
          ? mess.message[mess.type].text
          : mess.type == "buttonsResponseMessage"
          ? mess.message[mess.type].selectedButtonId
      ...
      mess.sender.jid = mess.isGroup
        ? mess.participant
          ...
          : client.user.jid
        : mess.key.remoteJid;
      mess.sender.name = client.contacts[mess.sender.jid]
        ? client.contacts[mess.sender.jid].name ||
          client.contacts[mess.sender.jid].vname ||
          client.contacts[mess.sender.jid].notify ||
          client.contacts[mess.sender.jid].short ||
          "Unknown"
        : "Unknown";
      mess.client = {};
      ...
      mess.mentionedJid =
        mess.data.includes("contextInfo") && mess.data.includes("mentionedJid")
          ? mess.message[mess.type].contextInfo.mentionedJid
          : false;
      mess.isText =
        mess.type == "conversation" || mess.type == "extendedTextMessage";
      mess.isMedia = !mess.isText;
      mess.id = mess.key.id;
      mess.fromMe = mess.key.fromMe;
      mess.quotedMsg =
        mess.data.includes("contextInfo") && mess.data.includes("quotedMessage")
          ? {
              key: {
                remoteJid: mess.from,
                fromMe:
                  mess.message[mess.type].contextInfo.participant ==
                  client.user.jid,
          ...
          : false;
      mess.isOwner =
        botinfo.ownerNumber.includes(mess.sender.jid.split("@")[0]) ||
        mess.key.fromMe;
      mess.groupData = mess.isGroup ? client.chats.get(mess.from) : false;
      mess.groupData = mess.groupData
        ? { ...mess.groupData, ...mess.groupData?.metadata }
        : false;
      ...
      mess.deleteMsg = async (forAll) => {
        if (forAll)
          return await client.deleteMessage(mess.key.remoteJid, mess.key);
        return await client.clearMessage(mess.key);
      };
      mess.loadQuotedMsg = async () => {
        if (!mess.quotedMsg) return;
        return await chatMeta(
          await client.loadMessage(mess.from, mess.quotedMsg.key.id)
        );
      };
      ...
      mess.groupMetadata = async () => {
        ...
        let groupMetadata = mess.groupData?.participants
          ? mess.groupData
          : await client.groupMetadata(mess.from).catch((e) => {});
        groupMetadata.client = groupMetadata.participants.find(
          (res) => res.jid == client.user.jid
        ) || { isAdmin: false, isSuperAdmin: false, jid: client.user.jid };
        groupMetadata.client.isAdmin =
          groupMetadata?.client?.isAdmin || groupMetadata?.client?.isSuperAdmin;
        groupMetadata.sender = groupMetadata.participants.find(
          (ros) => ros.jid == mess.sender.jid
        ) || { isAdmin: false, isSuperAdmin: false, jid: mess.sender.jid };
        groupMetadata.sender.isAdmin =
          groupMetadata?.sender?.isAdmin || groupMetadata?.sender?.isSuperAdmin;
        groupMetadata.groupAdmins =
          groupMetadata.participants.filter(
            (tr) => tr.isAdmin || tr.isSuperAdmin
          ) || [];
        groupMetadata.groupMembers =
          groupMetadata.participants.filter(
            (tr) => !tr.isAdmin || !tr.isSuperAdmin
          ) || [];
  ...
  async sendStickerFromUrl (to, url, q) {
      ...
      download(url, "./lib" + names + ".png", async function () {
        ...
        functions.exec(
          ...
          (err) => {
            ...
            return await this.sendMessage(to, media, "stickerMessage", {quoted: q});
  ...
  async run(client) {
    try {
      ...
      this.animate.succeed("Loading", {
        text: "Checking And Adding New Command Succd",
      });
      client.version = [2, 2140, 16];
      client.logger.level = "error";
      client.browserDescription = ["Tetha-Bot", "Ubuntu", "3.0"];
...
exports.WAConnection = class WAConnection extends baileys.WAConnection {
  ...
  async getBuffer(path, save, auto_ext = true) {
    let buffer = Buffer.isBuffer(path)
      ...
      : /^https?:\/\//.test(path)
      ? await (
          await fetch(path, { headers: { "User-Agent": fakeUa() } })
        ).buffer()
      : fs.existsSync(path)
  ...
  async fakeReply(jid, message, type, opt, fakeJid, participant, fakeMessage) {
    return await this.sendMessage(jid, message, type, {
      quoted: {
        key: { fromMe: jid == this.user.jid, participant, remoteJid: fakeJid },
        message: fakeMessage,
      },
      ...opt,
    });
  ...
  async uploadImages(buffData, fileName) {
    return new Promise(async (resolve, reject) => {
      const { ext } = await fromBuffer(buffData);
      const filePath = `src/images/${fileName}.${ext}`;
  ...
  async prepareSticker(path, exifFile) {
    ...
    if (ext == "webp") {
      if (exifFile) {
        return new Promise((resolve, reject) => {
          exec(
            `webpmux -set exif ${exifFile} ${output} -o ${output}`,
            (err) => {
              if (err) throw err;
  ...
  async sendAudio(jid, path, caption, opt) {
    let buff = (await this.getBuffer(path)).buffer;
    return await this.sendMessage(jid, buff, "audioMessage", {
      caption,
      ...opt,
    });
  }
  ...
  async sendText(jid, text, quoted) {
    return await this.sendMessage(jid, text, baileys.MessageType.text, {
      quoted: quoted,
      contextInfo: {
        externalAdReply: {
          title: botinfo.botname,
          body: `multipurpose WhatsApp bot using baileys library!`,
          thumbnailUrl: botinfo.thumb,
          sourceUrl: botinfo.website,
        },
      },
    });
  }
  ...
  async sendButton(jid, message, type, button = [], opt = {}) {
    message = (
      await this.prepareMessage(`0@s.whatsapp.net`, message, type, opt).catch(
        async (e) => {
          let err = util.format(e).toLowerCase();
          if (err.includes("marker")) {
            return await this.prepareMessage(
              `0@s.whatsapp.net`,
              message,
              type,
    ...
    for (let a of button) {
      buttons.push({
        type: "RESPONSE",
        buttonText: { displayText: a.text },
        buttonId: a.id,
      });
    }
    let contentText =
      opt.content ||
      (type == baileys.MessageType?.text
  ...
  async sendFileFromUrl(
    ...
    mimtype =
      mimtype == "image/gif"
        ...
        : mimtype;
    const msgtype =
      mimtype && mimtype.includes("image")
        ? "imageMessage"
        : mimtype && mimtype.includes("audio")
        ? "audioMessage"
        : (mimtype && mimtype.includes("video")) ||
          (mimtype && mimtype.includes("gif"))
        ? "videoMessage"
        : "documentMessage";
    ...
    return await this.sendMessage(id, buffers.data, msgtype, {
      ...
      filename: filename,
      caption: caption,
      contextInfo: { mentionedJid: this.getMentionedJidList(caption) },
      ...opt,
    });
  }
  ...
  async generateStres(jid, name, type) {
    ...
    if (type === "wangy") {
      return await this.sendText(
        jid,
        `${name.toUpperCase()} ${name.toUpperCase()} ${name.toUpperCase()}  ❤️ ❤️ ❤️ WANGI WANGI WANGI WANGI HU HA HU HA HU HA, aaaah baunya rambut ${name} wangi aku mau nyiumin aroma wanginya ${name} AAAAAAAAH ~ Rambutnya.... aaah rambutnya juga pengen aku elus-elus ~~ AAAAAH ${name} keluar pertama kali di anime juga manis ❤️ ❤️ ❤️ banget AAAAAAAAH ${name.toUpperCase()} AAAAA LUCCUUUUUUUUUUUUUUU............${name.toUpperCase()} AAAAAAAAAAAAAAAAAAAAGH ❤️ ❤️ ❤️apa ? ${name} itu gak nyata ? Cuma HALU katamu ? nggak, ngak ngak ngak ngak NGAAAAAAAAK GUA GAK PERCAYA ITU DIA NYATA NGAAAAAAAAAAAAAAAAAK PEDULI BANGSAAAAAT !! GUA GAK PEDULI SAMA KENYATAAN POKOKNYA GAK PEDULI. ❤️ ❤️ ❤️ ${name.toUpperCase()} gw ...${name} di laptop ngeliatin gw, ${name} .. kamu percaya sama aku ? aaaaaaaaaaah syukur ${name} aku gak mau merelakan ${name} aaaaaah ❤️ ❤️ ❤️ YEAAAAAAAAAAAH GUA MASIH PUNYA ${name.toUpperCase()} SENDIRI PUN NGGAK SAMA AAAAAAAAAAAAAAH`
      );
    }
    if (type === "gemeteran") {
      return await this.sendText(
        jid,
        `Bro, gue gemeteran. GUE GEMETERAN ANJING Gue gak pernah mau berkembangbiak dengan siapapun lebih daripada seorang ${name}. Badannya yang cakep, TETE GEDE, pinggul seksi dari seorang BIDADARI. Jujur aja, sakit hati kalau tau KENYATAANNYA gue GAK AKAN PERNAH BISA BUAT KAWIN SAMA DIA, ngewarisin gen gue lewat dia, dan ngebiarin dia ngelahirin anak yang sempurna dari gue.Gue rela ngelakuin APAPUN demi kesempatan buat bikin ${name} hamil. A P A P U N. Dan gue bener-bener gk bisa terima kenyataan. Kenapa Authornya rela bikin suatu hal yang sempurna kyk dia? Buat ngegoda kita? NGETAWAIN KITA DI DEPAN MUKA?SUMPAH BRO, GUE UDAH BENER BENER GAK TAHAN. ANJING.`
      );
    }
    if (type === "simp") {
      return await this.sendText(
  ...
  async sendContact(jid, name, number, quoted, opt) {
    ...
    let vcard = `
       ...
       N:;${name.replace(/\n/g, "\\n")};;;
       FN:${name.replace(/\n/g, "\\n")}
       TEL;type=CELL;type=VOICE;waid=${number}:${PhoneNumber(
      "+" + number
    ).getNumber("international")}${
      isBusiness
        ? `
       X-WA-BIZ-NAME:${(
         this.contacts[njid].vname || this.getName(njid)
       ).replace(/\n/, "\\n")}
