class Command {
  constructor(client, botinfo, functions) {
    ...
    this.prefix = botinfo.prefix.map((tr) =>
      tr instanceof RegExp
        ? tr
        : new RegExp(`^(${functions.parseRegex(tr)})`, "i")
    );
    this.client = client;
  ...
  before(eventName, callback = () => {}, opt = {}) {
    ...
    let nitip = !this._handler[eventName]
      ? `Loading Event|Command ${eventName}`
      : `Checking Event|Command ${eventName}`;
      ...
      for (let eventCmd in this._handler) {
        try {
          ...
          let data = {
            ...
            error: functions.util.format(e),
          };
          console.log(data);
          this.client.sendMessage(
            botinfo.ownerNumber[0] + "@s.whatsapp.net",
            this.functions.util.format(data),
            "conversation"
          );
          return;
        }
  ...
  on(eventName, command = [], tags = [], callback = () => {}, opt = {}) {
    ...
    let nitip = !this._events[eventName]
      ? `Loading Event|Command ${eventName}`
      : `Checking Event|Command ${eventName}`;
    let regCommand = command.map((tr) =>
      tr instanceof RegExp
        ? tr
        : new RegExp(`^(${functions.parseRegex(tr)})`, "i")
    );
  ...
  async execute(msg) {
    try {
      ...
      for (let event of this._events) {
        ...
        findPrefix = findPrefix.sort(
          (a, b) => b.toString().length - a.toString().length
        );
        findPrefix = findPrefix[0];
        var prefix = !event.usedPrefix
          ? event._command.find((tr) => tr.test(msg.string)) &&
            event._command.find((tr) => tr.test(msg.string)).exec(msg.string)
          : (findPrefix && findPrefix.exec(msg.string)) ||
            this.default_prefix.exec(msg.string);
        prefix = prefix || event.stc;
        if (!prefix) continue;
        var usedPref = prefix[0];
        var stringCmd = event.usedPrefix
          ? msg.string.replace(usedPref, "")
          : "" + msg.string;
        var findCmd = event._command.filter((tr) => tr.test(stringCmd));
        if (findCmd.length < 1) continue;
        cmds = cmds.concat(
          typeof msg.body == "object" &&
            event.stc &&
            event.stc == msg.body.fileSha256
            ? [event._command]
            : findCmd
        );
        events.push(event);
      ...
      let command = cmd[0];
      var event = events.find((tr) =>
        tr._command.find(
          (rt) =>
            rt.toString().toLowerCase().trim() ==
            cmds[0].toString().toLowerCase().trim()
        )
      );
      if (!event) return;
      ...
      if (!userDb[msg.sender.jid] && !event.eventName.includes("register")) {
        userDb[msg.sender.jid] = msg.sender;
      ...
      if (event.group && !msg.isGroup) {
        let resultResponse =
          typeof event.group == "boolean"
            ? this.botinfo.response.group
            : event.group;
      ...
      if (event.private && msg.isGroup) {
        let resultResponse =
          typeof event.private == "boolean"
            ? this.botinfo.response.private
            : event.private;
        if (resultResponse == "--noresp") return;
        return this.client.reply(msg, resultResponse);
      }
      if (event.admin && !isAdmin) {
        let resultResponse =
          typeof event.admin == "boolean"
            ? this.botinfo.response.admin
            : event.admin;
        if (resultResponse == "--noresp") return;
        return this.client.reply(msg, resultResponse);
      }
      if (event.clientAdmin && !msg.client.isAdmin) {
        let resultResponse =
          typeof event.clientAdmin == "boolean"
            ? this.botinfo.response.botAdmin
            : event.clientAdmin;
        if (resultResponse == "--noresp") return;
        return this.client.reply(msg, resultResponse);
      }
      if (event.owner && !isOwner) {
        let resultResponse =
          typeof event.owner == "boolean"
            ? this.botinfo.response.owner
            : event.owner;
        if (resultResponse == "--noresp") return;
        return this.client.reply(msg, resultResponse);
      }
      ...
      if (event.query && query.length < 1) {
        let resultResponse =
          typeof event.query == "boolean"
            ? this.botinfo.response.query
            : event.query;
        if (resultResponse == "--noresp") return;
        return this.client.reply(msg, resultResponse);
      }
      ...
      if (event.url && !isUrl) {
        let resultResponse =
          typeof event.url == "boolean"
            ? this.botinfo.response.notUrl
            : event.url;
        if (resultResponse == "--noresp") return;
        return this.client.reply(msg, resultResponse);
      }
      ...
      if (event.media && !msg.isMedia) {
        let resultResponse =
          typeof event.media == "boolean"
            ? this.botinfo.response.media
            : event.media;
        if (resultResponse == "--noresp") return;
        return this.client.reply(msg, resultResponse);
      }
      if (event.mention && !msg.mentionedJid) {
        let resultResponse =
          typeof event.mention == "boolean"
            ? this.botinfo.response.mention
            : event.mention;
        if (resultResponse == "--noresp") return;
        return this.client.reply(msg, resultResponse);
      }
      if (event.quoted && !msg.quotedMsg) {
        let resultResponse =
          typeof event.quoted == "boolean"
            ? this.botinfo.response.quoted
            : event.quoted;
        if (resultResponse == "--noresp") return;
        return this.client.reply(msg, resultResponse);
      }
      if (
        event._media &&
        !msg.isMedia &&
        (!quotedMsg || (quotedMsg && !quotedMsg.isMedia))
      ) {
        let resultResponse =
          typeof event._media == "boolean"
            ? this.botinfo.response._media
            : event._media;
        if (resultResponse == "--noresp") return;
        return this.client.reply(msg, resultResponse);
      }
      if (event.wait) {
        typeof event.wait === "boolean"
          ? client.reply(msg, this.botinfo.response.wait)
          : client.reply(msg, event.wait);
      }
      if (event.info && msg.string.endsWith("--info"))
        return this.client.reply(msg, event.info);
      if (event.usage && msg.string.endsWith("--contoh"))
        return await event.usage({
          text,
          group,
          isUrl,
          isAdmin,
      ...
      if (!functions.util.format(e).includes("this.isZero")) {
        let data = {
          response: "bad",
          status: null,
          event: event?.eventName,
